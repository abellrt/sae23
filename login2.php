<?php
	...
	if(empty($login) || empty($motdep))
	...
	else
     {
		...
		$requete = "SELECT `mot_de_passe` FROM `bâtiments` WHERE login='$login'";
		$resultat = mysqli_query($id_bd, $requete)
			or die("Execution de la requete impossible : $requete");

		$ligne = mysqli_fetch_row($resultat);
		if ($motdep==$ligne[0] AND $nom_bât == 'B')
		 {
			$_SESSION["auth"]=TRUE;		
            mysqli_close($id_bd);
			echo "<script type='text/javascript'>document.location.replace('Gestion_batB.php');</script>";
		 }
		else
		 {
